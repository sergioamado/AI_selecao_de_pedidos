...
def min_conflicts(csp, max_steps=100000, heuristic=None):
    """Solve a CSP by stochastic Hill Climbing on the number of conflicts."""
    # Generate a complete assignment for all variables (probably with conflicts)
    csp.current = current = {}
    for var in csp.variables:
        val = min_conflicts_value(csp, var, current, heuristic=heuristic)  # Passa a heurística
...
def min_conflicts_value(csp, var, current, heuristic=None):
    """Return the value that will give var the least number of conflicts.
    If there is a tie, choose at random."""
    if heuristic:
        return argmin_random_tie(csp.domains[var], key=lambda val: heuristic(csp, var, val, current))
    else:
